...
import { isAdminFailure } from "@/lib/cms/admin-redirect";
...
type QueuedFrame = { id: string; file: File; preview: string };
...
function GalleryBatchDrop({
  ...
  const enqueue = (list: FileList | File[]) => {
    ...
    if (tooLarge) {
      setError(formatAdminMessage(t.media.fileTooLarge, { max: String(maxMb) }));
    } else {
      setError("");
    }
  ...
  const uploadQueue = async () => {
    ...
    try {
      for (const item of snapshot) {
        ...
        const result = await addProjectGalleryFrameAction(fd);
