/** Official Yandex Metrika snippet (tag.js?id=…). */
export function getYandexMetrikaInitScript(counterId: string): string {
...
export function getYandexMetrikaNoscriptUrl(counterId: string): string {
  const id = Number(counterId);
  if (!Number.isFinite(id) || id <= 0) return "";
